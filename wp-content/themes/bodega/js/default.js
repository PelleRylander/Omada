...
function headerSize($scroll){
    ...
    //set control class for Select Slider in order to change header style
...
function logoWidth(){
	...
	if($j('.left_menu_position').length){
		...
		if($j('header').hasClass('sticky')){
			l_width = $j('header .q_logo a img.sticky').width();
			$j('header .logo_wrapper').css('width', l_width +'px');
		} else {
			l_width = $j('header .q_logo a img.normal').width();
			$j('header .logo_wrapper').css('width', l_width +'px');
		}
	}
}
	...
	if($j('.carousel').length){
		$j('.carousel').each(function(){
			...
			if($this.hasClass('full_screen')){
				...
				$this.find('.qode_slider_preloader').css({'height': ($j(window).height() - mobile_header - paspartu_amount_with_top_bottom - paspartu_amount_with_bottom) + 'px'});
                $this.find('.qode_slider_preloader .ajax_loader').css({'display': 'block'});
                ...
                $this.find('.qode_slider_preloader').css({'height': ($this.height() - mobile_header - paspartu_amount_with_top_bottom - paspartu_amount_with_bottom) + 'px', 'display': 'block'});
                if($j('.paspartu_outer:not(.disable_top_paspartu):not(.paspartu_on_bottom_slider)').length){
                    $this.closest('.q_slider').css('padding-top', Math.round(header_height_add_for_paspartu + $window_width * paspartu_width));
                    $this.closest('.q_slider').css('padding-bottom', Math.round($window_width * paspartu_width));
                }
...
function contentMenuScrollTo(){
	...
	if($j('nav.content_menu').length){
		...
		$j("nav.content_menu ul.menu li a").on('click', function(e){
			...
			var targetOffset = $j("div.wpb_row[data-q_id='" + $target + "'],section.parallax_section_holder[data-q_id='" + $target + "']").offset().top - content_height - content_menu_top - content_menu_top_add;
			$j('html,body').stop().animate({scrollTop: targetOffset }, 500, 'swing', function(){
				$j('nav.content_menu ul li').removeClass('active');
...
function initPopupMenu(){
    ...
    if($j('a.popup_menu').length){
        ...
        if($j('body').hasClass('fade_push_text_right') || $j('body').hasClass('fade_push_text_top')) {
            $j('.popup_menu_holder_outer nav a').each(function (i) {
                $j(this).css({
                    '-webkit-animation-delay': i * 70 + 'ms',
                    'animation-delay': i * 70 + 'ms'
                });
            });
            $j('.fullscreen_menu_widget_holder').css({
                '-webkit-animation-delay': ($j('.popup_menu_holder_outer nav a').length+1)*70 + 'ms',
                'animation-delay': ($j('.popup_menu_holder_outer nav a').length+1)*70 + 'ms'
            });
        }
