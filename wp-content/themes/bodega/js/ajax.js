...
function perPageBindings() {
    ...
    $j('.q_slider_preloader .ajax_loader').hide();
    content = $j('.content');
    initQodeSlider();
    initQodeCarousel();
    initMessageHeight();
...
function slideInNewPage(content, text, direction, direction2, animationTime, callbacks, url) {
    ...
    var animation;
    var header_style;
    var pageId = newHTML.find('#qode_page_id').text();
            ...
            if (animation === "ajax_updown" || animation === "ajax_updown_fade") {
                ...
                if (animation === "ajax_updown_fade") {
                    content.stop().fadeOut(animationTime, function () {
                        ...
                        setPageMeta(meta_data); // this function is called here since there need to be set new classes on body, before all function are called (ex. transparency class and qode slider width)
			...
			if($j('header.page_header').hasClass('fixed') && !$j('body').hasClass('vertical_menu_enabled')){
				...
				if(!$j('header.page_header').hasClass('transparent') || $j('header.page_header').hasClass('scrolled_not_transparent')) {
					...
					if (header_height - ($j('[data-q_id="' + hash + '"]').offset().top + top_header_height) / 4 >= min_header_height_scroll) {
						...
						scrollToAmount = $j('[data-q_id="' + hash + '"]').offset().top - min_header_height_scroll - large_menu_item_border;
					}
				}else{
